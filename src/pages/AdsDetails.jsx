...
const AdsDetails = () => {
    ...
    return (
      <main className="container mx-auto px-4 py-8">
        ...
        {status === "error" && (<div className='text-center m-2 text-error font-bold'>Failed to load property details.</div>)}
        {status === "success" && (
          <>
              ...
              <div className="lg:col-span-2 space-y-8">
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    <span className="inline-flex items-center gap-2 badge badge-lg bg-orange-500/10 text-orange-600 border-none font-bold px-4 py-3 uppercase tracking-wider text-xs">
                      <Tag size={14} /> {properties?.category}
                    </span>
                    <span className="text-sm font-medium text-base-content/40 inline-flex items-center gap-2"> <CalendarDays size={14} /> Published on {new Date(properties?.created_at).toLocaleDateString()}</span>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 bg-orange-500/5 rounded-3xl border border-orange-500/20">
                ...
                <div className="space-y-4">
                  <h3 className="text-2xl font-bold text-base-content border-l-4 border-orange-500 pl-4">Description</h3>
                ...
                <div className="pt-8 border-t border-base-200">
                  ...
                  {user ? (
                      ...
                      <div className="bg-orange-500/5 p-6 rounded-2xl border border-orange-500/20 mt-8 text-center">
                          <p className="text-base-content/80 font-medium">Please <Link to={`/login?next=${location.pathname}`} className="text-orange-600 font-bold hover:underline">login</Link> to leave a review.</p>
                      </div>
                  )}
                </div>
              </div>
              <div className="lg:col-span-1">
                <div className="sticky top-24 p-8 bg-base-100 rounded-[2.5rem] border border-orange-500/10 shadow-[0_20px_60px_-15px_rgba(255,165,0,0.1)]">
                  ...
                  <div className="space-y-3">
                    {user && !isOwner && requestStatus !== 'sent' && (
                        <button
                            ...
                            className="btn btn-lg bg-orange-500 hover:bg-orange-600 border-none text-white w-full rounded-2xl shadow-xl shadow-orange-500/20"
                        >
                    ...
                    {!user && ( 
                        <div className="bg-orange-500/10 text-orange-600 p-3 rounded-xl text-center font-bold">
                    ...
                    <div className="flex gap-2">
                      {properties?.contact_phone && <a href={`tel:${properties?.contact_phone}`} className="btn btn-outline border-orange-500/20 text-orange-600 flex-1 rounded-xl">
                        Call
                      </a>}
                      {properties?.contact_email && <a href={`mailto:${properties?.contact_email}`} className="btn btn-outline border-orange-500/20 text-orange-600 flex-1 rounded-xl">
                        Email
                      </a>}
                    </div>
                  </div>

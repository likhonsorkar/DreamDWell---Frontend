...
const Profile = () => {
    ...
    return (
        <div className="max-w-4xl mx-auto">
            {status === "wait" && (<div className='text-center m-2'><span className="loading loading-bars loading-xl text-primary"></span></div>)}
            <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-xl shadow-gray-100 border border-gray-100">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    {editable && (
                        <div className="md:col-span-2 mt-8">
                            <button type="button" onClick={() => setShowPasswordSection(!showPasswordSection)} className={`flex items-center gap-2 font-bold transition-all ${showPasswordSection ? 'text-red-500' : 'text-primary hover:text-primary'}`}>
                                {showPasswordSection ? '− Hide Password Settings' : '+ Change Password?'}
                            </button>
                            {showPasswordSection && (
                                <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6 p-6 mt-4 bg-primary/50 rounded-3xl border border-primary animate-in fade-in duration-300">
                                    <div className="form-control">
                                        <label className="label font-bold text-gray-600 text-sm">Current Password</label>
                                        <input type="password" placeholder="••••••••" {...register("password", { required: showPasswordSection, minLength: { value: 8, message: "Min 8 characters" } })} className={`input input-bordered w-full rounded-2xl bg-white border-gray-100 focus:border-primary ${errors.password ? 'border-red-500' : ''}`} />
                                        {errors.password && <span className="text-red-500 text-xs mt-1">{errors.password.message}</span>}
                                    </div>
                                    <div className="form-control">
                                        <label className="label font-bold text-gray-600 text-sm">New Password</label>
                                        <input type="password" placeholder="••••••••" {...register("newpassword", { required: showPasswordSection, minLength: { value: 8, message: "Min 8 characters" } })} className={`input input-bordered w-full rounded-2xl bg-white border-gray-100 focus:border-primary ${errors.newpassword ? 'border-red-500' : ''}`} />
                                    ...
                                    <div className="form-control md:col-span-2">
                                        ...
                                        <input type="password" placeholder="••••••••" {...register("cpassword", { required: showPasswordSection, validate: (val) => watch('newpassword') === val || "Your passwords do not match" })} className={`input input-bordered w-full rounded-2xl bg-white border-gray-100 focus:border-primary ${errors.cpassword ? 'border-red-500' : ''}`} />
                ...
                <div className="mt-12">
                    {!editable ? (
                        <button type="button" onClick={() => setEditable(true)} className="btn bg-primary hover:bg-primary border-none text-white w-full h-14 rounded-2xl text-lg font-bold shadow-xl shadow-primary transition-all active:scale-95">
                            Edit Profile
                        </button>
                    ) : (
